...
void initWeb()
{
  ...
  web.on(
         "/updatefw", HTTP_POST, [](AsyncWebServerRequest *request) { /*ws.textAll("X6");*/ /*This sends everyone the reboot command? Not sure why*/ },
         handle_fw_upload)
      .setFilter(ON_STA_FILTER);

  // WebSockets -> Web Request: X6 Reboot
  web.on("/reboot", HTTP_POST, [](AsyncWebServerRequest *request)
         { reboot = true; request->send(200, "text/plain", "Rebooting...."); });

  // WebSockets -> Web Request: XJ - Get RSSI,heap,uptime, e131 stats in json
  web.on("/stats", HTTP_GET, [](AsyncWebServerRequest *request)
         {
  ...
  // WebSockets -> Web Request: G2 - Get network status
  web.on("/networkStatus", HTTP_GET, [](AsyncWebServerRequest *request)
         {
           DynamicJsonDocument json(1024);
  ...
  // TODO: Fix reboot
  web.on(
         "/config", HTTP_POST, [](AsyncWebServerRequest *request) { /*ws.textAll("X6");*/ /*This sends everyone the reboot command? Not sure why*/ },
