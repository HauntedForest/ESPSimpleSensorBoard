/*
  ...
  Modified by Eric Golde -- Remove all debug statements.
*/
...
void AsyncHttpClient::send(bool debug)
{
    if(debug) { Serial.println("Type: " + _type + " URL: " + _fullUrl + " DataMode: " + _dataMode + " Data: " + _data); }

    if (_hostname.length() <= 0) {
		if(debug) { Serial.println("Hostname is not defined"); }
		return;
	}

    if (_port <= 0) {
		if(debug) { Serial.println("Port is not defined"); }
		return;
	}

    if (_type.length() <= 0) {
		if(debug) { Serial.println("Type is not defined (Only GET or POST)"); }
		return;
	}

    if (_fullUrl.length() <= 0) {
		if(debug) { Serial.println("URL is not defined"); }
		return;
	}
    ...
    {
        if(debug) { Serial.println("Connect Fail"); }
